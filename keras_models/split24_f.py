...
class MultiDecoder(tf.keras.layers.Layer):
    def __init__(self, n_layers_en, n_layers_de, time_dim, embed_dim, out_dim, num_heads, ff_dim, drop_rate,
        ...
        self.encoderf = TransformerEncoderBlock(n_layers_en, out_dim, embed_dim, 6, num_heads, ff_dim, drop_rate,
                                               batch_size)
        self.decoderf1 = TransformerDecoderBlock(n_layers_de, out_dim, embed_dim, 6, num_heads, ff_dim, drop_rate,
                                               batch_size)
        self.decoderf2 = TransformerDecoderBlock(n_layers_de, out_dim, embed_dim, 6, num_heads, ff_dim, drop_rate,
                                               batch_size)
        self.decoderf3 = TransformerDecoderBlock(n_layers_de, out_dim, embed_dim, 6, num_heads, ff_dim, drop_rate,
                                               batch_size)
    ...
    def call(self, inputs, look_ahead_mask):
        ...
        inputs = tf.transpose(inputs, perm=[0, 2, 1])  # [B, outdim, timedim]
        encoderf_in = inputs[:, 0:self.out_dim, 0:6]
        encoderf_out = self.encoderf(encoderf_in)
        # 第一层decoder
        decoderf_in1 = inputs[:, 0:self.out_dim, 6:12]
        decoderf_out1, w1 = self.decoderf1(decoderf_in1, encoderf_out, look_ahead_mask=mask2[1])
        # 第2层decoder
        last_in = decoderf_out1
        decoderf_in2 = inputs[:, 0:self.out_dim, 12:18]
        decoderf_out2, w2 = self.decoderf2(decoderf_in2, last_in, look_ahead_mask=mask2[1])
        # 第3层decoder
        last_in = decoderf_out2
        decoderf_in3 = inputs[:, 0:self.out_dim, 18:24]
        decoderf_out3, w3 = self.decoderf3(decoderf_in3, last_in, look_ahead_mask=mask2[1])
