...
class MultiDecoder_f(tf.keras.layers.Layer):
    ...
    def call(self, inputs, look_ahead_mask):
        inputs = inputs[:, :, 0:self.time_dim]
        inputs = tf.transpose(inputs, perm=[0, 2, 1])  # [B, outdim, timedim]

        encoder_out = self.encoder(inputs)
        decoder_out, weights = self.decoder(inputs, encoder_out, look_ahead_mask)

        decoder_out = tf.transpose(decoder_out, perm=[0, 2, 1])
...
class MultiDecoder(tf.keras.layers.Layer):
    def __init__(self, n_layers_en, n_layers_de, time_dim, embed_dim, out_dim, num_heads, ff_dim, drop_rate,
        ...
        self.decoder3 = TransformerDecoderBlock(n_layers_de, time_dim, embed_dim, out_dim, num_heads, ff_dim, drop_rate,
                                                batch_size)
        # self.decoder_f = MultiDecoder_f(n_layers_en, n_layers_de, out_dim, embed_dim, 48, num_heads, ff_dim,
        #                                 drop_rate, batch_size)

    def call(self, inputs, look_ahead_mask):
        encoder_in = inputs[:, 0:6, 0:self.out_dim]
        encoder_out = self.encoder(encoder_in)
        # 第一层decoder
        decoder_in1 = inputs[:, 6:12, 0:self.out_dim]
        decode_out1, w1 = self.decoder1(decoder_in1, encoder_out, look_ahead_mask)
        # print("decoderlayer1```")
        # 第2层decoder
        last_in = decode_out1
        decoder_in2 = inputs[:, 12:18, 0:self.out_dim]
        decode_out2, w2 = self.decoder2(decoder_in2, last_in, look_ahead_mask)
        # print("decoderlayer2```")
        # 第3层decoder
        last_in = decode_out2
        decoder_in3 = inputs[:, 18:24, 0:self.out_dim]
        decode_out3, w3 = self.decoder3(decoder_in3, last_in, look_ahead_mask)
        # print("decoderlayer3```")
        # mask_input2 = tf.random_uniform((self.batch_size, self.out_dim))
        # mask2 = make_masking(mask_input2, mask_input2)
        # x2, w_f = self.decoder_f(inputs, look_ahead_mask=mask2[1])

        # output = tf.concat([decode_out3, x2], 1)
        output = decode_out3
...
class Network(tf.keras.Model):
    def __init__(self, batch_size, dim, batch_norm, dropout, rec_dropout, task,
        ...
        # self.transformer = Transformer(n_layers_en, n_layers_de, split_time, embed_dim, out_dim, num_heads, ff_dim,
        #                                drop_rate, batch_size)
        self.decoder = MultiDecoder(n_layers_en, n_layers_de, self.split_time, embed_dim, self.out_dim, num_heads, ff_dim,
                                    drop_rate, self.batch_size)
    ...
    # def call(self, X)
        ...
        mask_input1 = tf.random.uniform((self.batch_size, self.split_time))
        mask_input2 = tf.random.uniform((self.batch_size, self.out_dim))
    ...
    def say_name(self):
        return "{}.n{}{}{}{}.dep{}".format('k_tf_split24',
                                           self.dim,
